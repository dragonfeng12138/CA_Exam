...
public class MyAdminTmRe extends JPanel implements ActionListener{

	JLabel jLabel = new JLabel("修 改 题 目 信 息");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(obj == l1t) {
			...
			if(s1.length() > 0) {
				...
				mst.setEditable(true);
				trt.setEditable(true);
				flt.setEditable(true);
				at.setEditable(true);
				bt.setEditable(true);
				ct.setEditable(true);
				dt.setEditable(true);
			...
			if(dat.getText().equals("A") || dat.getText().equals("a")) sda = 1;
				...
				JOptionPane.showMessageDialog(this, "‘正确答案格’式不对错误！");
				return;
			}
			Tm tm = new Tm(sid, sname, sms, str, sfl, sa, sb, sc, sd, sda);
			if(bean.update(no, tm))
